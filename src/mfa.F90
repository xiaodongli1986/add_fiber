program MFA
  ...
  integer   ::  Ndata,Nrand,nn1,nn2,sigbin,pibin,tbin,bin,wgal,wran,q
  double precision :: aux,dr,odr,theta,var,aux1,aux2
...
!print*,sum(wgt1(1:Ndata)),sum(wgt1(1:Ndata))/float(Ndata)
!print*,sum(wgt1(Ndata+1:Ndata+Nrand)),sum(wgt1(Ndata+1:Ndata+Nrand))/float(Nrand)

wgt1(1:Ndata)=float(Ndata)*wgt1(1:Ndata)/sum(wgt1(1:Ndata))
wgt1(Ndata+1:Ndata+Nrand)=float(Ndata)*wgt1(Ndata+1:Ndata+Nrand)/sum(wgt1(Ndata+1:Ndata+Nrand))
!print*,sum(wgt1(1:Ndata)),sum(wgt1(1:Ndata))/float(Ndata)
!stop
...
do i=1,Ndata,4
   ...
   call kdtree2_r_nearest_around_point(tp=tree,idxin=i,correltime=-1,r2=bins(k,2)*bins(k,2),&
...
!               else
!                  Zdd(k,1)=Zdd(k,1)+
                  aux1=aux1+wgt1(resultsb(j)%idx)!*wgt1(i))
!                  print*,'aux1',aux1
!               endif
!            else
!               if(resample) then
...
!                else
!                  Zdr(k,1)=Zdr(k,1)+
                  aux2=aux2+wgt1(resultsb(j)%idx)!*wgt1(i))
!                  print*,'aux2',aux2
!               endif
!            else
!               if(resample) then
...
!                  Zdr(k,1)=Zdr(k,1)+1.d0
 !              endif
   ...
   enddo
      ...
      if(aux1==0.0 .or. aux2==0.0) print*, 'im dead!', nn2,aux1,aux2
      
      do q=1,11
          qq=float(q-6)
          Zdd(k,q)=Zdd(k,q)+4*4E-4 *(((bins(k,2)**3.)*(aux1/float(Ndata))/aux2)**(qq-1.))/float(Ndata)
      enddo  
...
do i=1,nbins
   write(11,'(12(e14.7,1x))') bins(i,2),Zdd(i,:)
!   print*,bins(i,1),bins(i,2), Zdd(i,1)
enddo
close(11)
endif
...
wran=0
   ...
   call kdtree2_r_nearest_around_point(tp=tree,idxin=i,correltime=-1,r2=600.0,&
&       nfound=nn2,nalloc=(Nrand+Ndata),results=resultsb)
