...
!! Now normalising initial weights at input - is this ok?
!call normalise_counts()
call load_save_randoms()
         ...
         case ('-proj')
            ...
            elseif(trim(arg)=='.false.') then
              proj=.false.
                ...
                stop "Error using -proj flag. Either .true. or .false."
            endif
         case('-iso')
            ISOFLAG = trim(arg)
         case('-decp') 
...
do i=1,Ndata
if(mkd(i)<aux-0.*sqrt(var) .and. mkd(i).ne.0.0) then
mkd(i)=1.0!/mkd(i)
else
mkd(i)=0.0
endif
