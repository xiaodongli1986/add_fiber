...
!--- allocate some initial memory ---
  ...
  Allocate(vel(d,Ndata+Nrand))
  
!  print*, (0.5*(float(Ndata)+1.)**2)
  if ( wgt )  allocate(wgt1(Ndata))
  allocate(mkd(Ndata))
  mkd(:)=1.0
         ...
         if(resultsb(j)%idx <=Ndata) then
            ...
            if ( wgt ) then
               ...
               if(resample) then
                  ...
                  Zdd(bin,1)=Zdd(bin,1)+(wgt1(resultsb(j)%idx)*wgt1(i))*mkd(i)
               endif
            ...
            if ( wgt ) then
                if(resample) then
                  ...
                  Zdr(bin,1)=Zdr(bin,1)+(wgt1(resultsb(j)%idx)*wgt1(i))*mkd(i)
                endif
            else
              if(resample) then
...
open(11,file=outfile,status='unknown')
if (resample) then
write(11,'(A)')'#  cos(theta)_min, cos(theta)_max, Rmin, Rmax,XI_LS, error'
else
write(11,'(A)')'#  cos(theta)_min,        cos(theta)_max,          Rmin,                   Rmax,                &
         ...
         if(resultsb(j)%idx <=Ndata) then            
            if ( wgt ) then
               if(resample) then
                  ...
                  Zdd(k,1)=Zdd(k,1)+(wgt1(resultsb(j)%idx)*wgt1(i))*mkd(i)
               endif
            else
               if(resample) then
            ...
            if ( wgt ) then
               if(resample) then
                  ...
                  Zdr(k,1)=Zdr(k,1)+(wgt1(resultsb(j)%idx)*wgt1(i))*mkd(i)
               endif
            else
               if(resample) then
...
!      call sph2cart(1.0,aux1,aux2,my_array(1,i),my_array(2,i),my_array(3,i))
    ...
    read(11,*,err=31,end=14)my_array(1:3,i),vel(1:3,i)
...
if ( wgt ) then
   nrr=sum(wgt1(Ndata+1:Ndata+Nrand))**2
    if(marked) then 
        ndd=sum(wgt1(1:Ndata)*mkd(:))*sum(wgt1(1:Ndata))
        ndr=sum(wgt1(1:Ndata)*mkd(:))*sqrt(nrr)
    else
...
subroutine marked_samples()
radius=20.0
v=4.0/3.0 * 3.14159 * radius**3
totv=768.0**3
den=float(Ndata)/totv
nex=den*v
...
wran=0
   ...
   call kdtree2_r_nearest_around_point(tp=tree,idxin=i,correltime=-1,r2=radius**2,&
&       nfound=nn2,nalloc=(Nrand+Ndata),results=resultsb)
...
!        endif
!    enddo  
!    if(wgal>1 .and. wran>1 .and. (float(wgal)/float(wran))<0.1) then
!       mkd(i)=1.0/(float(wgal)/float(wran))
!    else
!        mkd(i)=1.0   
!    endif
 
!    if(wgal>1 .and. wran>1 ) then
!       mkd(i)=(float(wgal)/float(wran))
!    else
!       mkd(i)=0.0   
!    endif 
...
aux=sum(mkd)/float(Ndata)
var=sum((mkd-aux)**2)/size(mkd)

print*, 'av den=',aux,var


do chunk=1,40

rmin=2.0 + chunk*3.
rmax=rmin+0.5/sqrt(float(chunk))

wgt1=0.0
k=0
do i=1,Ndata

!if(mkd(i)>aux+20*var) goto 33

   call kdtree2_r_nearest_around_point(tp=tree,idxin=i,correltime=-1,r2=rmax**2,&
& nfound=nn2,nalloc=(Nrand+Ndata),results=resultsb)

    nn1=kdtree2_r_count_around_point(tp=tree,idxin=i,correltime=-1,r2=rmin**2)

        do j=nn1+1,nn2,1
        if(mkd(resultsb(j)%idx)>aux-2*var) goto 33
        v2=my_array(1:3,resultsb(j)%idx)
        v1=vel(1:3,resultsb(j)%idx)

        k=k+1
        dist=sqrt((my_array(1,i)-v2(1))**2 + (my_array(2,i)-v2(2))**2 + (my_array(3,i)-v2(3))**2)

        vd=(vel(1,i)-v1(1))*(my_array(1,i)-v2(1)) + &
 &         (vel(2,i)-v1(2))*(my_array(2,i)-v2(2)) + &
 &         (vel(3,i)-v1(3))*(my_array(3,i)-v2(3))       
        vd=vd/dist
        wgt1(k)=vd
...
print*, rmin,sum(wgt1)/float(k),k
enddo

stop

do i=1,Ndata
if(mkd(i)<aux-sqrt(var) .and. mkd(i).ne.0.0) then
mkd(i)=1.0!/mkd(i)
else
mkd(i)=0.0
endif
